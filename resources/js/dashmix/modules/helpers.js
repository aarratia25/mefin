...
export default class Helpers {
    ...
    static run(helpers, options = {}) {
        let helperList = {
            ...
            "core-bootstrap-custom-file-input": () =>
                this.coreBootstrapCustomFileInput(),
    ...
    static coreToggleClass() {
        jQuery('[data-toggle="class-toggle"]:not(.js-class-toggle-enabled)')
            ...
            .on("click.pixelcave.helpers.core", e => {
                ...
                // Toggle class
                jQuery(el.data("target").toString()).toggleClass(
                    el.data("class").toString()
                );
            });
    }
    ...
    static coreYearCopy() {
        ...
        if (el.length > 0) {
            ...
            el.addClass("js-year-copy-enabled").html(
                parseInt(baseYear) >= curYear
                    ? curYear
                    : baseYear + "-" + curYear.toString().substr(2, 2)
            );
        }
    }
                ...
                jQuery(".js-table-sections-header > tr", table).on(
                    ...
                    e => {
                        if (
                            ...
                            !jQuery(e.target).parent(".custom-control")
                                .length &&
                            ...
                            if (!tbody.hasClass("show")) {
                                jQuery("tbody", table).removeClass(
                                    "show table-active"
                                );
                            }
    ...
    static tableToolsCheckable() {
        // For each table
        jQuery(".js-table-checkable:not(.js-table-checkable-enabled)").each(
            (index, element) => {
                let table = jQuery(element);
                ...
                jQuery("thead input:checkbox", table).on(
                    "click.pixelcave.helpers",
                    e => {
                        let checkedStatus = jQuery(e.currentTarget).prop(
                            "checked"
                        );
                ...
                jQuery("tbody input:checkbox, tbody input + label", table).on(
                    "click.pixelcave.helpers",
                    e => {
                        ...
                        if (!checkedStatus) {
                            jQuery("thead input:checkbox", table).prop(
                                "checked",
                                false
                            );
                        } else {
                            if (
                                jQuery("tbody input:checkbox:checked", table)
                                    .length ===
                                jQuery("tbody input:checkbox", table).length
                            ) {
                                jQuery("thead input:checkbox", table).prop(
                                    "checked",
                                    true
        ...
        if (jQuery.isEmptyObject(options)) {
            ...
            jQuery.notify(
                ...
                },
                {
                    ...
                    allow_dismiss:
                        options.allow_dismiss === false ? false : true,
                    newest_on_top:
                        options.newest_on_top === false ? false : true,
        ...
        jQuery(".js-maxlength:not(.js-maxlength-enabled)").each(
            (index, element) => {
                ...
                el.addClass("js-maxlength-enabled").maxlength({
                    ...
                    warningClass:
                        el.data("warning-class") || "badge badge-warning",
                    limitReachedClass:
                        el.data("limit-reached-class") || "badge badge-danger",
            ...
            (index, element) => {
                ...
                let types = {
                    line: () => {
                        ...
                        options["lineColor"] =
                            el.data("line-color") || "#0665d0";
                        options["fillColor"] =
                            el.data("fill-color") || "#0665d0";
                        options["spotColor"] =
                            el.data("spot-color") || "#495057";
                        options["minSpotColor"] =
                            el.data("min-spot-color") || "#495057";
                        options["maxSpotColor"] =
                            el.data("max-spot-color") || "#495057";
                    ...
                    pie: () => {
                        ...
                        options["highlightLighten"] =
                            el.data("highlight-lighten") || 1.1;
                        options["tooltipFormat"] =
                            "{{prefix}}{{value}}{{suffix}}";
                    },
                    tristate: () => {
                        ...
                        options["barSpacing"] = el.data("bar-spacing") || 6;
                        options["posBarColor"] =
                            el.data("pos-bar-color") || "#82b54b";
                        options["negBarColor"] =
                            el.data("neg-bar-color") || "#e04f1a";
                    }
                };
                ...
                if (types[type]) {
                    ...
                    if (type === "line") {
                        if (
                            el.data("chart-range-min") >= 0 ||
                            el.data("chart-range-min")
                        ) {
                            options["chartRangeMin"] = el.data(
                                "chart-range-min"
                            );
                        }

                        if (
                            el.data("chart-range-max") >= 0 ||
                            el.data("chart-range-max")
                        ) {
                            options["chartRangeMax"] = el.data(
                                "chart-range-max"
                            );
                        }
                    }
    ...
    static storage(config = {}) {
        if (localStorage.getItem("config") == null) {
            ...
            let globalObject = JSON.parse(localStorage.getItem("config"));

            Object.assign(globalObject, config);

            localStorage.setItem("config", JSON.stringify(globalObject));
        }
    }
